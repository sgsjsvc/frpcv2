...
func toolInfoSize() uint32 {
	// Older Common Controls builds reject the modern TOOLINFO size that includes
	// the reserved pointer field. Using the v1 size keeps tooltip registration
	// compatible across both legacy and manifest-enabled environments.
	return uint32(unsafe.Offsetof(win.TOOLINFO{}.LpReserved))
}
